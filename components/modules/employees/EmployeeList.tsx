...
export function EmployeeList({
  ...
  const filteredEmployees = employees.filter((emp) => {
    ...
    return matchesSearch && matchesDept && matchesStation;
  });

  const totalBasicSalary = employees.reduce((s, e) => s + Number(e.basicSalary), 0);
  const totalDueToEmployees = employees.reduce(
    (s, e) => s + (e.remaining > 0 ? e.remaining : 0),
    0
  );
  const totalAdvancesOnEmployees = employees.reduce(
    (s, e) => s + (e.remaining < 0 ? Math.abs(e.remaining) : 0),
    0
  );
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-3">
            ...
            <span>إدارة الموظفين والرواتب (Employees & Payroll)</span>
          </h1>
          <p className="text-xs text-gray-500 mt-1">
            إدارة بيانات الكادر الإداري والفني ومتابعة كشوف الحسابات والسلف وصرف الرواتب
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
          <div className="flex justify-between items-center text-gray-500 mb-1">
            <span className="text-xs font-semibold">إجمالي الموظفين المسجلين</span>
          ...
          <span className="text-[11px] text-gray-400">
            {employees.filter((e) => e.status === "نشط").length} على رأس العمل
          </span>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
          <div className="flex justify-between items-center text-gray-500 mb-1">
            <span className="text-xs font-semibold">صافي مستحقات الموظفين (لهم)</span>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
          <div className="flex justify-between items-center text-gray-500 mb-1">
            <span className="text-xs font-semibold">سلف وعهد قائمة (عليهم)</span>
        ...
        <table className="w-full text-right text-xs">
          <thead className="bg-gray-50 text-gray-700 font-bold border-b border-gray-200">
            <tr>
              ...
              <th className="p-3.5 font-mono">الراتب الأساسي</th>
              <th className="p-3.5 font-mono">الرصيد المالي الحالي</th>
            ...
            {filteredEmployees.length === 0 ? (
              ...
              filteredEmployees.map((emp) => (
                <tr key={emp.id} className="hover:bg-gray-50/70 transition-colors">
                  ...
                  <td className="p-3.5 text-gray-700 font-medium">{emp.position}</td>
