...
def build_nav(current_filename):
    ...
    for fname, label, icon in pages_info:
        # Match current file or employees alias
...
for root, dirs, files in os.walk(base_dir):
    # skip subfolders _1.._9 if any, process root html
    if root != base_dir:
        continue
    for file in files:
        if file.endswith(".html"):
            ...
            if re.search(nav_pattern, content, re.DOTALL):
                ...
                with open(file_path, "w", encoding="utf-8") as f:
                    f.write(content)
                print(f"Successfully updated nav sidebar in {file}")

print("COMPLETED SIDEBAR SYNC ACROSS ALL PAGES.")
