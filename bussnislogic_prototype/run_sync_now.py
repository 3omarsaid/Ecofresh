...
def build_nav(current_filename):
    ...
    for fname, label, icon in pages_info:
        ...
        style_cls = 'bg-secondary-container text-on-secondary-container font-bold border-r-4 border-surface-variant' if is_active else 'text-on-primary hover:bg-primary-fixed-variant/40 transition-colors'
...
for f in os.listdir(base_dir):
    if f.endswith('.html'):
        ...
        with open(path, 'r', encoding='utf-8') as file:
            content = file.read()
        start_marker = '<nav class="fixed right-0 top-0 h-screen w-[280px]'
...
print(f'Done updating {updated} files.')
