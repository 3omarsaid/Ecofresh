...
def generate_nav(current_file):
    ...
    return f'''<!-- SideNavBar Navigation -->
    ...
    for file in files:
        if file.endswith(".html"):
            ...
            if start_idx != -1 and end_idx != -1:
                end_idx += len("</nav>")
                new_nav_str = generate_nav(file)
                updated_content = content[:start_idx] + new_nav_str + content[end_idx:]
                
                with open(file_path, "w", encoding="utf-8") as f:
                    f.write(updated_content)
                count += 1
                print(f"FORCED NAV INJECTED: {file}")

print(f"SUCCESS. Injected nav into {count} HTML files.")
